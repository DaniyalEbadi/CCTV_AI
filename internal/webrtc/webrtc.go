...
// protect PeerConnection creation
var pcMu sync.Mutex
...
func Init() {
	...
	// client uses same API unless we have a specific address
	clientAPI = serverAPI
	if address != "" {
	...
	// thread-safe PeerConnection creation
...
// --- asyncHandler and ExchangeSDP remain mostly unchanged
// --- only minor safety/logging enhancements added
	...
	conn.Listen(func(msg any) {
		...
		case *pion.ICECandidate:
			if !FilterCandidate(msg) {
				log.Trace().Str("candidate", msg.Address).Msg("[webrtc] filtered ICE candidate")
				return
			}
			_ = sendAnswer.Wait()
			s := msg.ToJSON().Candidate
			log.Trace().Str("candidate", s).Msg("[webrtc] local ICE ")
